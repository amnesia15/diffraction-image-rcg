...
def set_parser_arguments(parser):
    ...
    parser.add_argument('-i', '--iterations', required=False,
                        default=8,
                        help='number of training iteration for the same '
                             'architecture')

    # List of number of units per hidden layer
    parser.add_argument('-hl', '--hidden_layers', required=False,
    ...
    parser.add_argument('-ts', '--test_split', required=False,
                        default=0.2,
    ...
    parser.add_argument('-dr', '--dropout_rates', required=False,
    ...
    parser.add_argument('-es', '--early_stopping', required=False,
                        default=1,
                        help='early stopping (0 - no early stopping, 1 - '
                             'using early stopping)')

    # Activation function
    parser.add_argument('-a', '--activation_func', required=False,
                        default='relu',
...
class Model:
    def __init__(self, regularization, h_layers, learning_rate, activation_func, dropout_rates):
    ...
    def _create_model(self,
        ...
        if regularization == 0:
            ...
            raise ValueError(f"""Wrong regularization value. Regularization
    ...
    def create_instance(self):
        ...
        """
        return self._create_model(self.regularization,
                                  self.h_layers,
                                  self.learning_rate,
...
class SessionStatistic:
    ...
    def write_statistics_to_file(self, directory_path):
        ...
        """
        errors_arr = np.vstack((self.mae_training,
                                self.mae_test,
                                self.rmse_training,
                                self.rmse_test,
                                self.mae_r_training,
        ...
        errors_df.columns = ['MAE training', 'MAE test', 'RMSE training',
                             'RMSE test', 'MAE R training', 'MAE R test',
                             'MAE H training', 'MAE H test', 'RMSE R training',
                             'RMSE R test', 'RMSE H training', 'RMSE H test']
    ...
    def write_mae_statistic_to_file(self, directory_path):
        ...
        # Params
            directory_path: Directory path for model outputs
        """
        r_list = [self.mean_mae_r_training, self.mean_mae_r_test,
                  self.std_mae_r_training, self.std_mae_r_test]
        h_list = [self.mean_mae_h_training, self.mean_mae_h_test,
                  self.std_mae_h_training, self.std_mae_h_test]
        both_list = [self.mean_mae_training, self.mean_mae_test,
                     self.std_mae_training, self.std_mae_test]

        maes_df = pd.DataFrame(np.array((r_list, h_list, both_list)))
        maes_df.columns = ['mean training', 'mean test',
                           'std training', 'std test']
    ...
    def write_rmse_statistic_to_file(self, directory_path):
        ...
        # Params
            directory_path: Directory path for model outputs
        """
        r_list = [self.mean_rmse_r_training, self.mean_rmse_r_test,
                  self.std_rmse_r_training, self.std_rmse_r_test]
        h_list = [self.mean_rmse_h_training, self.mean_rmse_h_test,
                  self.std_rmse_h_training, self.std_rmse_h_test]
        both_list = [self.mean_rmse_training, self.mean_rmse_test,
                     self.std_rmse_training, self.std_rmse_test]

        rmaes_df = pd.DataFrame(np.array((r_list, h_list, both_list)))
        rmaes_df.columns = ['mean training', 'mean test',
                            'std training', 'std test']
...
def _plot_metric_function(metric, metric_val, iteration, directory_path):
    ...
    plt.legend()
    plt.savefig(os.path.join(directory_path, f'iter_{iteration}', 'metric_training.png'))
    plt.close()


def _plot_prediction_scatter_plot(x, y, xlabel, ylabel, title, iteration,
                                  directory_path, file_output, plt_low, plt_high):
...
def train(model, data, params, test_split, batch_size,
          epochs, iterations, ind_early_stopping, model_out):
    """Trains the model.
    ...
    for i in range(0, iterations):
        ...
        train_X, test_X, train_y, test_y = train_test_split(data, params, test_size=test_split)

        keras_model = model.create_instance()

        print('[INFO] training model')
        H = keras_model.fit(train_X,
                            train_y,
        ...
        session_statistic.rmse_r_training.append(
            sqrt(mean_squared_error(trainY_scaled[:, 0], predicted_training_scaled[:, 0])))
        session_statistic.rmse_h_training.append(
            sqrt(mean_squared_error(trainY_scaled[:, 1], predicted_training_scaled[:, 1])))
        ...
        _plot_metric_function(H.history['mean_absolute_error'],
        ...
        np.savetxt(os.path.join(model_out, f'iter_{i}', 'predictions.csv'),
                   predictions, delimiter=',', fmt='%.2f')

        print('[INFO] ploting scatter plot of predictions')
        # Ploting the scatter plot of predictions for R
        _plot_prediction_scatter_plot(x=testY_scaled[:, 0], y=predictions[:, 0],
                                      xlabel="Real R", ylabel="Predicted R",
                                      title="Predicted vs Real R", iteration=i,
                                      directory_path=model_out, file_output="pred_vs_realR.png",
                                      plt_low=1000, plt_high=5000)

        # Ploting the scatter plot of prediction for H
        _plot_prediction_scatter_plot(x=testY_scaled[:, 1], y=predictions[:, 1],
                                      xlabel="Real H", ylabel="Predicted H",
                                      title="Predicted vs Real H", iteration=i,
                                      directory_path=model_out, file_output="pred_vs_realH.png",
                                      plt_low=100, plt_high=10000)
...
def run_train(args):
    ...
    model_class = Model(regularization=regularization,
                        h_layers=h_layers,
